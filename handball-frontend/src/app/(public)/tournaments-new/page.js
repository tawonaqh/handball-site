...
export default function TournamentsPage() {
  ...
  useEffect(() => {
    async function fetchTournaments() {
      try {
        const data = await fetcher("leagues"); // Backend endpoint stays as leagues
        setTournaments(data || []);
      } catch (error) {
        ...
        <div className="container mx-auto px-6">
          ...
          >
            ...
            <p className="text-xl text-gray-600 max-w-2xl mx-auto">
              Discover all the handball tournaments across Zimbabwe and follow your favorite teams
            </p>
          </motion.div>
          ...
          {tournaments.length === 0 && (
            <div className="text-center py-20">
              ...
              <p className="text-gray-500">Check back later for tournament updates</p>
            </div>
          )}
        </div>
      </div>
