...
export default function EditRefereePage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [refereeData, tournamentsData] = await Promise.all([
          fetcher(`referees/${refereeId}`),
          fetcher('tournaments')
        ]);
        ...
        setFormData({
          ...
          is_active: refereeData.is_active ?? true,
          photo_url: refereeData.photo_url || ""
        });
        setTournaments(tournamentsData || []);
      } catch (error) {
        console.error('Error loading data:', error);
        alert('Failed to load referee data');
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        router.push(`/admin/referees/${refereeId}`);
      } else {
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        ...
        >
          ...
          <p className="text-gray-400 text-lg">Loading referee data...</p>
        </motion.div>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      >
        <div className="flex items-center gap-4">
          <Link href={`/admin/referees/${refereeId}`}>
          ...
          <div>
            ...
            <p className="text-gray-400 mt-1">Update referee information</p>
          </div>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="flex gap-4 pt-4">
          ...
          >
            ...
            {loading ? 'Updating...' : 'Update Referee'}
          </button>
          <Link href={`/admin/referees/${refereeId}`} className="flex-1">
            <button
              type="button"
