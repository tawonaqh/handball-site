...
export default function EditPlayerPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const playerResponse = await fetch(`${API_URL}/players/${id}`);
        ...
        setFormData({
          ...
          team_id: playerData.team_id?.toString() || playerData.team?.id?.toString() || '',
        ...
        setErrors({ load: 'Failed to load player data' });
      } finally {
        setLoading(false);
      }
    }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setErrors({ submit: error.message || 'An error occurred while updating the player' });
    } finally {
      setSaving(false);
    }
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 text-lg">Loading player data...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="bg-white rounded-xl shadow-sm p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-600 mb-6">
            {errors.load || 'The player you are looking for does not exist or has been removed.'}
          </p>
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Edit Player</h1>
              <p className="text-gray-600">
                Update the details for <span className="font-semibold text-blue-600">{player.name}</span>
              </p>
            </div>
            <button
              onClick={() => router.push('/admin/dashboard')}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              <h3 className="text-sm font-semibold text-gray-700 mb-2 flex items-center space-x-2">
                <span>ℹ️</span>
                <span>Original Player Information</span>
              </h3>
              <div className="text-sm text-gray-600 space-y-2">
                ...
                <div><strong>Matches Played:</strong> {player.matches_played || 0}</div>
                <div><strong>Created:</strong> {player.created_at ? new Date(player.created_at).toLocaleDateString() : 'N/A'}</div>
                <div><strong>Last Updated:</strong> {player.updated_at ? new Date(player.updated_at).toLocaleDateString() : 'N/A'}</div>
              </div>
            </div>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mt-6">
              <h3 className="text-sm font-semibold text-gray-700 mb-2 flex items-center space-x-2">
                <span>💡</span>
                <span>About Player Management</span>
              </h3>
              <ul className="text-sm text-gray-600 space-y-1">
                <li>• Update player statistics to reflect their current performance</li>
                <li>• Changing teams will transfer the player to a different roster</li>
                <li>• Position helps identify player role in the team formation</li>
                <li>• Bio can include recent achievements, strengths, or background</li>
              </ul>
            </div>
          </form>
        </div>
